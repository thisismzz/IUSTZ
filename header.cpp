...
void Skills::upgradeSkill(BankAccount *creditcard){
    try{
        ...
        cout<<"not enough money! You shoould kill more enemy and steal their money. \n"; // Prints an error message if there's not enough money
    } 
}
...
void Backpack::ConsumeFoodForSurvival() {
    ...
    if(FoodItems.empty()){
        cout << "No food items left in the backpack." << endl;
        foodMenu();
    }
...
void Throwable::buy(int quantity){
    ...
    if(creditcard->withdraw(price*quantity)){
        ...
        cout<<"Not enough money! maube later\n"; // Prints a message if there's not enough money to buy the item
}
...
void getUserInfo(int& age , string& gender , string& username) {
    ...
    cout << "What do they call you ? (enter your name)" << endl;
    cin >> username;
    cout <<  endl << "and you are older than 15 right? If not fuck off!!! (enter your age)" << endl;
    cin >> age;
    if (age < 15){
        cout << endl << "Sorry kid , we don`t have time to deal with your parents` complaint." << endl << "END GAME!!" << endl;
        Sleep(3000);
        exit(0);
    }
    ...
    cout << "REMEBER WINTER IS COMING" << endl;
